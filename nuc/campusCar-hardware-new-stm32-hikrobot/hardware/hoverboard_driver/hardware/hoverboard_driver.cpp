...
namespace hoverboard_driver
{
  ...
  hoverboard_driver_node::hoverboard_driver_node(const std::string &node_name) : Node(node_name)
  {
      ...
      const hardware_interface::HardwareInfo &info)
  {
    ...
    if (info_.hardware_parameters.count("command_limit_rpm"))
    {
      command_limit_rpm_ = std::stod(info_.hardware_parameters["command_limit_rpm"]);
    }
    if (!std::isfinite(command_limit_rpm_) || command_limit_rpm_ <= 0.0)
    {
      RCLCPP_WARN(
          rclcpp::get_logger("hoverboard_driver"),
          "Invalid command_limit_rpm %.3f; falling back to 50 RPM", command_limit_rpm_);
      command_limit_rpm_ = 50.0;
    }
    if (command_limit_rpm_ > 1000.0)
    {
      RCLCPP_WARN(
          rclcpp::get_logger("hoverboard_driver"),
          "command_limit_rpm %.3f exceeds seller protocol range; clamping at 1000 RPM", command_limit_rpm_);
      command_limit_rpm_ = 1000.0;
    }
    if (info_.hardware_parameters.count("feedback_format"))
    {
      const std::string feedback_format = info_.hardware_parameters["feedback_format"];
      compact_feedback_ = feedback_format != "full";
    }
    ...
    // Use hardware name to distinguish node names when multiple instances run (e.g. front/rear)
    std::string node_name = "hoverboard_driver_node_" + info_.name;
    // Replace non-alphanumeric chars with underscore for valid node name
    for (auto &c : node_name) if (!std::isalnum(c) && c != '_') c = '_';
    hardware_publisher = std::make_shared<hoverboard_driver_node>(node_name);
      ...
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("hoverboard_driver"), "Serial command limit: %.1f RPM", command_limit_rpm_);
  ...
  void hoverboard_driver::protocol_recv(const rclcpp::Time &time, char byte)
  {
    ...
    const size_t feedback_size = compact_feedback_ ? sizeof(SerialFeedbackCompact) : sizeof(SerialFeedback);

    // Read the start frame
    if ((start_frame == START_FRAME || start_frame == START_FRAME_IMU) && msg_len == 0)
    {
      receiving_feedback_frame_ = start_frame == START_FRAME;
      receiving_imu_frame_ = start_frame == START_FRAME_IMU;
      if (start_frame == START_FRAME)
      {
        if (compact_feedback_)
        {
          p = (char *)&compact_msg;
        }
        else
    ...
    else if (msg_len >= 2)
    {
      ...
      size_t active_target_size =
          receiving_feedback_frame_ ? feedback_size :
          receiving_imu_frame_ ? sizeof(SerialImu) :
          0;
      if (msg_len < active_target_size)
      {
        *p++ = byte;
        msg_len++;
      }
      else if (active_target_size == 0)
      {
        msg_len = 0;
        receiving_feedback_frame_ = false;
        receiving_imu_frame_ = false;
      }
    }

    if (compact_feedback_ && msg_len == sizeof(SerialFeedbackCompact) && compact_msg.start == START_FRAME)
    {
      uint16_t checksum = (uint16_t)(compact_msg.start ^
                                     compact_msg.cmd1 ^
                                     compact_msg.cmd2 ^
                                     compact_msg.speedR_meas ^
                                     compact_msg.speedL_meas ^
                                     compact_msg.batVoltage ^
                                     compact_msg.boardTemp ^
                                     compact_msg.cmdLed);

      if (compact_msg.checksum == checksum)
      {
        hardware_publisher->publish_voltage((double)compact_msg.batVoltage / 100.0);
        hardware_publisher->publish_temp((double)compact_msg.boardTemp / 10.0);

        // Convert signed RPM to RAD/S. The campusCar STM32 reply has no
        // encoder ticks, so odometry is configured open-loop.
        hw_velocities_[left_wheel] = direction_correction * ((double)compact_msg.speedL_meas * 0.10472);
        hw_velocities_[right_wheel] = direction_correction * ((double)compact_msg.speedR_meas * 0.10472);
      ...
      {
        RCLCPP_WARN(rclcpp::get_logger("hoverboard_driver"), "Hoverboard compact checksum mismatch: %d vs %d", compact_msg.checksum, checksum);
      }
      msg_len = 0;
      receiving_feedback_frame_ = false;
      receiving_imu_frame_ = false;
    }
    else if (!compact_feedback_ && msg_len == sizeof(SerialFeedback) && msg.start == START_FRAME)
    {
      ...
      if (msg.checksum == checksum)
      {
        ...
        // Convert signed RPM to RAD/S
        hw_velocities_[left_wheel] = direction_correction * ((double)msg.speedL_meas * 0.10472);
        hw_velocities_[right_wheel] = direction_correction * ((double)msg.speedR_meas * 0.10472);
        hardware_publisher->publish_vel(left_wheel, hw_velocities_[left_wheel]);
        hardware_publisher->publish_vel(right_wheel, hw_velocities_[right_wheel]);
    ...
    else if (msg_len == sizeof(SerialImu) && imu_msg.start == START_FRAME_IMU)
    {
      ...
      {
        hardware_publisher->publish_imu(imu_msg, time);
      }
      else
      {
        RCLCPP_WARN(rclcpp::get_logger("hoverboard_driver"), "Hoverboard IMU message checksum mismatch: %d vs %d", imu_msg.checksum, checksum);
      }
      msg_len = 0;
      receiving_feedback_frame_ = false;
      receiving_imu_frame_ = false;
      ...
      const rclcpp::Time & /*time*/, const rclcpp::Duration &period)
  {
    ...
    // Convert wheel commands from rad/s to RPM for the seller-confirmed
    // 0xABCD steer/speed protocol.
    double set_speed[2] = {
    ...
    if (!write_serial_command(clamp_command_value(steer), clamp_command_value(speed)))
    {
  ...
  int16_t hoverboard_driver::clamp_command_value(double value) const
  {
    const double limited = std::clamp(value, -command_limit_rpm_, command_limit_rpm_);
    return static_cast<int16_t>(std::lround(limited));
  }

  bool hoverboard_driver::write_serial_command(int16_t steer, int16_t speed)
  {
    ...
    const int rc = ::write(port_fd, (const void *)&command, sizeof(command));
    return rc == (int)sizeof(command);
  }
